<template>
	<div id="imgShow" class="img_show" v-show="true">
		<div class="img_show_main" :style="{height: img_main_h + 'px'}">
			<div class="menuBar"
				style="line-height:40px; min-width: 190px;position: relative;left:.3em;margin-top: 1em;">
				<li class="parentListMenu" v-for="(ele,key) in parentMenu" :key="key"
					style="margin-bottom: .2em;list-style:none;line-height:40px;cursor:pointer;font-size:16px;background-color:rgb(242 242 242);border: 1px solid #BFD0E0;">
					<div style="display:flex;align-items: center;" class="parentMenuContainer"
						:id="'parentMenuName'+ele.id" @click="toMainNav(ele.id)">
						<img :src="ele.icon" alt="" width="25px" height="25px" />
						<p class="parentMenuName"> <span data-v-03ee4ecd="" class=""
								style="width: 3px; height: 18px; background-color: rgb(187, 188, 189); margin-right: 1em;"></span>{{ele.name}}
						</p>
					</div>
					<ul :id="ele.id+'_ul'" class="subli" style="">
						<li :class="{'activeSel':menuItem.id==ele.id}" :id="e.id" class="subMenuItem"
							v-if="ele.id==e.pid" v-for="(e,indx) in allMenuData" :key="indx"
							style="list-style:none;cursor:pointer" @click="toSubNav(e)">
			...
			<div class="show_imgs_box">
					...
					<div class="img_show_tools">
						...
						<div class="tools-control-speed">
							...
							<div class="control-speed">
								<el-slider v-model="timesSpeed" :step="0.2" :min="0.2" :max="1.5" :marks="marks"
									show-stops @change="changeSpeed"></el-slider>
							</div>
						</div>
					</div>
				...
				<div style="display:flex;margin-left:.4em;">
					<div class="img_timeline" style="width:15vw;margin-top: 1em;">

						<div class="img_startTime"
							style="padding:1em;background-color: rgb(242, 242, 242);margin-bottom:.3em;border: 1px solid #BFD0E0;">
							<p style="text-align:center;font-size: 16px;margin-bottom:.1em">起报时间</p>
							<el-date-picker v-model="timeValue" type="datetime" format="yyyy-MM-dd HH:mm:ss"
								@change="changeDate" size="mini"></el-date-picker>
						</div>
						<div class="img_forecastInterval" id="img_forecastInterval"
							style="padding:1em;background-color: rgb(242, 242, 242);margin-bottom:.3em;border: 1px solid #BFD0E0;">
							...
							<div class="time_interval" style="display:flex;justify-content: center;font-size: 14px;">
								<p :class="{'activeSel':imgTime==item}"
									style="margin-right:1em;padding:.3em;cursor:pointer;padding-left:.5em;padding-right:.5em;padding-top:.2em;padding-bottom:.2em"
									v-for="(item,index) in imgTimeInterval" :key="index" @click="getIntervalItem(item)">
									{{item}}</p>
							</div>
						</div>
						<div class="img_forcecasttime" 
							style="padding:1em;text-align: center;background-color: rgb(242, 242, 242);border: 1px solid #BFD0E0;  ">
							...
							<div style="display:flex;align-items: center;padding-bottom: .4em;justify-content: center;">
								<div class="show_imgs_timeline" style="padding-right: 1.6em;">
									<div class="hl_title">
										<h5>
											<span class="timePlayBtn"
												:class="isPause ? 'img_timePlay' : 'img_timePause'"
												@click="play"></span>
							...
							<transition name="slide-fade" :duration="timesSpeed">
								<ul style="display:flex;flex-wrap: wrap;margin-left:0em">
									<li class="forcecastItem" :class="{'activeSel':imgValid==item.V_RETAIN1_C}"
										style="width:28%;list-style:none;border:1px solid #d3d3d3;padding:.2em;cursor:pointer"
										v-for="(item,index) in interval_list" :key="index"
										@click="getforcecastItem(item,index)">{{item.V_RETAIN1_C }}</li>
								</ul>
							</transition>
						</div>
					</div>
						...
						<div style="display:flex;justify-content: space-around;margin-top:1em;">
							<div style="display:flex;align-items: center;border: 1px solid black;">
								...
								<div style="display:flex;">
									<p :class="{'activeSel':imgModel==item}"
										style="margin-left:.5em;cursor:pointer;padding:.4em"
										v-for="(item,index) in imgModelArr" :key="index" @click="getImgModel(item)">
										{{item}}</p>
								</div>
							...
							<div style="display:flex;align-items: center;border: 1px solid black;">
								...
								<div style="display:flex;">
									<p :class="{'activeSel':imgRegion==item}"
										style="margin-left:.5em;cursor:pointer;padding:.4em"
										v-for="(item,index) in imgRegionArr" :key="index" @click="getImgRegion(item)">
										{{item}}</p>
								</div>
							</div>
						</div>
...
<script>
	...
	import {
		startLoading,
		endLoading
	} from '../../../config/comsConfig/ele-ui/el-loading.js';
	...
	export default {
		...
		computed: {
			...
			sortFutureHrs: function() {
				imgData.forEach((imgItem) => {
					if (imgItem.Datetime) {
						timeTransform_temp.getAppointDateStr(new Date(timeTransform_temp.toTimeZone(imgItem
							.Datetime.replace(/\s/g, '').replace(/[^a-zA-Z0-9]/g, ''), 8,
							'yyyy/MM/dd hh:mm:ss')), parseInt(imgItem.V_RETAIN1_C), 'MM/dd hh:mm')
					}
				})
        ...
        */
		...
		beforeDestroy: function() {
			if (document.querySelector('#hl_content') != null) document.querySelector('#hl_content')
				.removeEventListener('mousewheel', this.handleScroll) || document.querySelector('#hl_content')
			...
			*/
			highMainlightColor(id) {
				...
				setTimeout(() => {
					...
					document.getElementById('parentMenuName' + id).style.borderBottom =
						"2px solid rgb(84, 193, 252)"
				}, 1000);
			...
			highSublightColor(id) {
				...
				if (this.prevSubId.startsWith('Nqtx-T00-M03')) {
					document.getElementById("img_forecastInterval").style.display='none'
				}else{
					document.getElementById("img_forecastInterval").style.display='block'
				}
        ...
        */
			getImgData(menuItem) {
				...
				*/
				...
				if (this.changedate) {
					...
					//params.timeRange = `(${this.timeTransform_temp.getAppointHourStr(this.timeValue[0], -8, 'yyyyMMddhhmmss')},${this.timeTransform_temp.getAppointHourStr(this.timeValue[1], -8 , 'yyyyMMddhhmmss')})`;
					params.times = 20230424120000
					this.timeValue = '2023-04-24 12:00:00'
			...
			setCss() {
				this.imgShowDivDom.css({
					top: '46px',
					left: $('#leftMenu').width(),
					width: document.body.offsetWidth - $('#leftMenu').width(),
					height: document.body.offsetHeight - 46
				});
			...
			handleScroll(e) {
				...
				$('.hTimes').animate({
					'marginTop': this.defaultPositionTimes - (parseInt(this.hTime_liStyle.height) * this
						.hoverIndex)
				}, {
					queue: false,
					duration: this.timesSpeed
				});
				this.hTimesStyle['margin-top'] = this.defaultPositionTimes - (parseInt(this.hTime_liStyle.height) * this
					.hoverIndex);
			},
